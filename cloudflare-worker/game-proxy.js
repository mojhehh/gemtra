...
export default {
    async fetch(request, env, ctx) {
        ...
        if (url.pathname.startsWith('/play/')) {
            const targetUrl = url.pathname.slice(6) + url.search;
...
(function() {
    ...
    console.log('[Proxy] Initialized with base:', ORIGINAL_BASE);
    ...
    function proxyUrl(url) {
        ...
        try {
            ...
            if (url.startsWith('//')) {
                ...
                // Relative path - resolve from base (includes directory)
                absolute = new URL(url, ORIGINAL_BASE).href;
            }
            
            console.debug('[Proxy] Resolving:', url, '->', absolute);
            return PROXY_BASE + '/p/' + encodeUrl(absolute);
        } catch (e) {
            console.warn('[Proxy] URL conversion failed:', url, e);
            return url;
        }
    }
    ...
    window.fetch = function(input, init) {
        ...
        if (shouldProxy(url)) {
            ...
            console.debug('[Proxy] fetch:', url, '->', proxiedUrl);
    ...
    XMLHttpRequest.prototype.open = function(method, url, ...args) {
        if (shouldProxy(url)) {
            const proxiedUrl = proxyUrl(url);
            console.debug('[Proxy] XHR:', url, '->', proxiedUrl);
            url = proxiedUrl;
        }
    ...
    // Note: This doesn't catch all location changes but helps
    try {
        const locationDescriptor = Object.getOwnPropertyDescriptor(window, 'location');
        // Can't override location reliably, but we can catch some patterns
    } catch (e) {}
    
    console.log('[Proxy] URL override handlers installed');
})();
</script>`;
}
